...
class Card {
	...
	addListeners() {
		...
		this.cardElement.querySelector('.place-card__description')
			.addEventListener('mousemove', this.showTooltip.bind(this));
		this.cardElement.querySelector('.place-card__description')
	...
	like() {
		if (!this.cardElement.querySelector('.place-card__like-icon').classList.contains('place-card__like-icon_liked')) {
			...
			this.cardElement
				.querySelector('.place-card__like-counter')
				.textContent = +this.cardElement.querySelector('.place-card__like-counter').textContent + 1;

			this.likes.push(document.querySelector('.user-info__name').textContent);

			api.doLike(this.cardElement.dataset._id)
				.then(card => this.cardElement
								.querySelector('.place-card__like-counter')
			...
			api.removeLike(this.cardElement.dataset._id)
				.then(card => this.cardElement
								.querySelector('.place-card__like-counter')
								.textContent = card.likes.length)
				.then(() => this.cardElement
								.querySelector('.place-card__like-icon')
								.classList.toggle('place-card__like-icon_liked'))
				.then(() => this.likes
								.splice(this.likes.indexOf(document.querySelector('.user-info__name').textContent), 1))
				.catch(err => console.error(`Ошибка регистрации лайка: ${err}`));
		}
	}
