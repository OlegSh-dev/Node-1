	...
	sortCards() {
		...
		if (document.querySelector('.sort-button').classList.contains('pressed')) {
			cardsArr.sort((card1, card2) => {
				return +card2.querySelector('.place-card__like-counter')
							.textContent - +card1.querySelector('.place-card__like-counter').textContent;
			});
		} else {
			cardsArr.sort((card1, card2) => {
				return +card1.querySelector('.place-card__like-counter')
							.textContent - +card2.querySelector('.place-card__like-counter').textContent;
			});
		}
